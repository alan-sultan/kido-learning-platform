...
import 'colors_completion_screen.dart';
import 'colors_routes.dart';
import 'colors_theme.dart';

class ColorsActivityScreen extends StatefulWidget {
  const ColorsActivityScreen({super.key, required this.lessonId});

  final String lessonId;

  @override
  State<ColorsActivityScreen> createState() => _ColorsActivityScreenState();
}

class _ColorsActivityScreenState extends State<ColorsActivityScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      backgroundColor: ColorsTheme.darkBackground,
      body: SafeArea(
        child: Center(
...
class _ActivityExperienceState extends State<_ActivityExperience> {
  ...
  Widget build(BuildContext context) {
    ...
    final total = ColorsLibrary.lessons.length;
        ...
        Expanded(
          child: SingleChildScrollView(
            ...
            child: Column(
              ...
              children: [
                Text(
                  'Can you find ${widget.metadata.displayName}?',
                ...
                if (_showSuccess)
                  Padding(
                    ...
                    child: _SuccessBadge(
                      label: 'You found ${widget.metadata.displayName}! 🎉',
                    ),
                  ),
              ],
            ),
  ...
  Future<void> _handleSelection(ColorActivityOption option) async {
    if (_isSubmitting) return;
    setState(() => _selectedOption = option.id);
    if (!option.isCorrect) {
      NavigationService.showSnackBar(
        SnackBar(content: Text('${option.label} is not the right color.')),
      );
      return;
    }
  ...
  Future<void> _completeLesson() async {
    ...
    Navigator.pushReplacement(
      context,
      MaterialPageRoute(
        settings: const RouteSettings(name: ColorsRoutes.completion),
        builder: (_) => ColorsCompletionScreen(
          lessonId: widget.metadata.lessonId,
        ),
      ),
    );
...
class _ActivityCard extends StatelessWidget {
  ...
  final ColorActivityOption option;
  final bool selected;
        ...
        child: Row(
          children: [
            ...
            Icon(
              ...
              color: selected ? option.accentColor : Colors.white38,
            )
          ],
        ),
      ),
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        borderRadius: BorderRadius.circular(999),
        border: Border.all(color: ColorsTheme.accentSun, width: 2),
      ),
      child: Row(
        mainAxisAlignment: MainAxisAlignment.center,
        children: [
          const Icon(Icons.auto_awesome, color: ColorsTheme.accentSun),
